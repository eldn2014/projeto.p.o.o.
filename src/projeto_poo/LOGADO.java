...
public class LOGADO {
  ...
  private int x;
  private int y;
  private String [][] dadosend01;
  private String [][] dadosend02;
    ...
    public boolean getLoginInfermeira(String Login, String senha) {
        x=6; y=3;
  ...
  public void dados01( String nome, String sus, String idade, String cpf, String email,String tele, String login ,String senha){
   ...
   public void atualn(){
    ...
    public void impr(int x, int y, String dadosend[][]){
      ...
      for(int i=0;i<x;i++){ 
         ...
         System.out.println("OS DADOS DA POISÇÃO ("+(i)+","+(j)+") "+ dadosend[i][j]);
  ...
  public void dados02(String rua, String bairro, String numero, String cidade, String estado,String cep){
      
      atualn2();
         
         if(x<2){
             
             dadosend02 = new String[x][y];
             
             for (int i=0; i<x; i++){
                 for (int j=0; j<y;j++){
                  if (j==0){dadosend02[i][j]= rua;}
                  if (j==1){dadosend02[i][j]= bairro;}
                  if (j==2){dadosend02[i][j]= numero;}
                  if (j==3){dadosend02[i][j]= cidade;}
                  if (j==4){dadosend02[i][j]= estado;}
                  if (j==5){dadosend02[i][j]= cep;}
                
             }}
         JOptionPane.showMessageDialog(null,"Os dados inseridos foram salvos", "Dados Salvos", JOptionPane.INFORMATION_MESSAGE);}
         ...
         dadosend01 =new String[x][y];
            ...
            for (int j=0; j<y;j++){
                  if (j==0){dadosend02[x-1][j]= rua;}
                  if (j==1){dadosend02[x-1][j]= numero;}
                  if (j==2){dadosend02[x-1][j]= cidade;}
                  if (j==3){dadosend02[x-1][j]= estado;}
                  if (j==4){dadosend02[x-1][j]= bairro;}
                 }
         
        for(int i=0;i<x-1;i++){ 
           for(int j=0; j<y-1;j++){
             
          dadosend02[i][j]=tmp[i][j] ; }}
